...
--
-- Provides 8 default document templates for PDF generation:
--   - Medical Certificate (IT/EN)
--   - Referral Letter (IT/EN)
--   - Lab Request (IT)
--   - Visit Summary (IT)
--   - Prescription (IT/EN)
--
-- Uses ON CONFLICT (template_key) DO NOTHING for idempotent re-runs.
-- created_by/updated_by are NULL (system-seeded, not user-created).
--
-- RLS is temporarily disabled during seeding to ensure INSERTs succeed
-- regardless of the current RLS context, then re-enabled at the end.
-- ==============================================================================
